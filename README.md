...
## Extension Points (For Interview)

### 1. Add New Request Type

Edit `ServiceRequest.java`:
```java
public enum RequestType {
    INCIDENT, REQUEST, CHANGE, FEATURE_REQUEST  // Add FEATURE_REQUEST
}
```

### 2. Implement New Assignment Strategy

```java
public class CustomAssignmentStrategy implements AssignmentStrategy {
    public Operator assign(ServiceRequest request, List<Operator> available) {
        // Your custom logic
    }
}
```

### 3. Modify BPMN Process

Edit `service-request-process.bpmn20.xml`:
- Add parallel approval gateway
- Add new user tasks
- Modify routing logic
- Change SLA timers

### 4. Configure Different SLA Policy

Edit `application.properties`:
```properties
app.sla.incident.triage=10
app.sla.incident.work=240
app.sla.change.review=120
```

### 5. Integrate External System

Extend `NotificationService`:
```java
private void sendToExternalSystem(Notification notification) {
    // Call REST API, Kafka, RabbitMQ, etc.
}
```
...
1. **Production-Ready** - Uses proven Flowable BPMN engine
2. **Extensible** - Easy to modify without refactoring core logic
3. **Clean Code** - Service layer, repositories, DTOs, proper separation of concerns
4. **Testable** - Mocked dependencies, isolated unit tests
5. **Observable** - Complete audit trail and event logging

Designed for rapid live modifications during the interview.
